...
export default function EditHeroPage({ params }: { params: { id: string } }) {
  ...
  const fetchHero = async () => {
    try {
      const response = await fetch(`/api/heroes/${params.id}`)
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Успешно",
          description: "Герой успешно обновлен",
        })
        ...
        toast({
          title: "Ошибка",
          description: error.error || "Не удалось обновить героя",
          variant: "destructive",
        })
      }
    } catch (error) {
      toast({
        title: "Ошибка",
        description: "Произошла ошибка при обновлении героя",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold">Редактировать героя</h1>
      </div>
